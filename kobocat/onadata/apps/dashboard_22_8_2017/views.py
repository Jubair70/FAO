...
def getDashboardDatatable(query):
    ...
    for eachval in fetchVal:
        ...
        delete_button = ''
        edit_button = '<a class="tooltips" data-placement="top" data-original-title="Edit Program" href="#" onclick="edit_entity(' + eachval[0] + ');"><i class="fa fa-2x fa-pencil-square-o"></i></a>' + ' '
    ...
    #def generate_json_column_area_chart(self, dataframe):
        ...
        if df.empty == False:
            if "colors" not in df:
                ...
                df = pd.pivot_table(df, values=data_field, rows=[name_field, "colors", "name_sorting"], cols=["category_sorting", category_field])
                df = df.fillna(0)

                # Check in Name Sorting order defined or not
                appearance = json.loads(self.dashboardGenerator.chart_object)
                name_sorting_ascending = True
