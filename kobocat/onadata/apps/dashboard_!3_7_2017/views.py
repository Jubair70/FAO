...
def generate_json_column_area_chart(name_field, category_field, data_field, query):
    ...
    :return:
    """
    dataset = utility_functions.__db_fetch_values_dict(
        query)  ## ******  ( Category for multiple value of each Legend) ******* (Start)    '''
    uniqueList = getUniqueValues(dataset, name_field)
    category_list = getUniqueValues(dataset, category_field)
    seriesData = []
    print category_list, "   uniqueList:  ",uniqueList
    color_category={}
    ...
    for ul in uniqueList:
        ...
        for category in category_list:
            color_category[category] = 0

        for nameTodata in dataset:
            if nameTodata[name_field] == ul:
                color_category[nameTodata[category_field]]=nameTodata[data_field]
        dict['data'] = color_category.values()
        seriesData.append(dict)
    print seriesData;
    jsonForChart = json.dumps({'categories': category_list, 'seriesdata': seriesData}, default=decimal_default)
    return jsonForChart
...
def generate_graph(request, graph_id):
    ...
    if dashboardGenerator.content_type == 0:  # GRAPH
        ...
        if dashboardGenerator.datasource_type == "1":  # 1 = QUERY
            return HttpResponse(json.dumps(getClusteredMap(dashboardGenerator.datasource)))
    ...
    def execute(self):
        ...
        if self.chart_def.js_code is not None:
            if "@parameter" in self.chart_def.js_code:
                ...
                filter_caller=filter_caller.replace("@parameter", "parameter")
...
def generate_dynamic_report(request):
    ...
    for eachrow in navigationBarParent:
        ...
        if not navigationBarChild:
            ...
            content_tabs += ' <div class="mpower-section middle-top" id="top_' + str(
                eachrow.id) + '"> </div><div data-load="unloaded" class="middle-container" id="container_' + str(
            ...
            for eachchildrow in navigationBarChild:
                ...
                content_tabs += '<div class="mpower-section middle-top"  id="top_' + str(
                    eachchildrow.id) + '" >  </div><div data-load="unloaded" class="middle-container" id="container_' + str(
