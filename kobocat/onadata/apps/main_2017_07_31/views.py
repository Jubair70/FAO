...
def profile(request, username):
    ...
    if request.method == 'POST' and request.user.is_authenticated():
        ...
        if form_result['type'] == 'alert-success':
            ...
            # when we want to enable sms check for all.
            # until then, it checks if form barely related to sms
            xform = get_object_or_404(XForm, user__username__iexact=username, id_string__exact=form_result['form_id_string'])
            print '________________________________________________________________'
            #print form_result['form_id_string']
            owner = xform.user
            #tree = ET.ElementTree(xform.xml)
