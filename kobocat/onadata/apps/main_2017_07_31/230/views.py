...
def edit(request, username, id_string):
    ...
    if username == request.user.username or\
        ...
        if request.POST.get('description'):
            ...
            elif request.POST['toggle_shared'] == 'dynamic':
                print 'sharing dynamic';
                xform.enable_dynamic_data = not xform.enable_dynamic_data
...
def enketo_preview(request, username, id_string):
    ...
    print 'enketo_preview_url: '+enekto_preview_url
...
@login_required
def survey_summary(request,username):   
    surveyData = {}
    totalSubmission = []
    surveyObj = []
    xforms = XForm.objects.filter(user__username__iexact=username)
    ...
    for xform in queryset_iterator(xforms):
        xformObj = get_object_or_404(XForm, id_string=xform)
        surveyObj.append(xformObj)
        form_id = str(xformObj.id_string)
        #submission_date_query = Instance.objects.raw('SELECT json->>_submission_time AS day_of_submission FROM logger_instance WHERE xform_id=%s', [form_id])
        submission_date_query = "SELECT (json->>'_submission_time')::timestamp::date AS day_of_submission FROM logger_instance WHERE xform_id="+str(xformObj.id)+""
        #submission_date_query = "SELECT to_date(json->>'_submission_time','DDMMYYYY') AS day_of_submission FROM logger_instance WHERE xform_id="+str(xformObj.id)+""
       
        #submission_date_dictionary = _execute_query(submission_date_query,False)
        cursor.execute(submission_date_query)
       # submission_date_dictionary = _dictfetchall(cursor)
        #json_dump_submission_date = json.dumps(cursor.fetchall())
        fetchVal = cursor.fetchall();
        #print fetchVal
        date = []
        for each in fetchVal:
...
def sendMessageSubscriber(request, username):
    ...
    message = Message_Queue(subscribeid=SUBSCRIPTION_ID,status=NEW,response="nothing else matter",msg_type=TYPE_MSG)
    message.save()
    message2 = Message_Queue(subscribeid=SUBSCRIPTION_ID,status=NEW,response="sending msg",msg_type=TYPE_SMS)
    message2.save()
    message = Message_Queue(subscribeid=SUBSCRIPTION_ID,status=NEW,response="to be or not to be that is the question",msg_type=TYPE_SMS)
    message.save()
    message2 = Message_Queue(subscribeid=SUBSCRIPTION_ID,status=NEW,response="testing type",msg_type=TYPE_EMAIL)
    message2.save()
    output = 'success'
    variables = RequestContext(request, {
