...
from onadata.apps.usermodule.models import UserModuleProfile, UserPasswordHistory,Organizations,OrganizationDataAccess,MenuItem,FaoDesignations
...
class UserProfileForm(forms.ModelForm):
    ...
    designation = forms.ModelChoiceField(label='Designation',required=True,queryset=FaoDesignations.objects.all(),empty_label="Select a Designation")
    # expired = forms.DateTimeField(label="Expiry Date",required=False,initial=datetime.now()+ timedelta(days=90))
    mobile_no = forms.CharField(label="mobile_no")
    dls_officer_registration_no = forms.CharField(label="dls_officer_registration_no",required=False)
    bvc_officer_registration_no = forms.CharField(label="bvc_officer_registration_no",required=False)
    class Meta:
        model = UserModuleProfile
        #fields = ('admin','employee_id','organisation_name','country','designation','psu')
        fields = ('admin','employee_id','organisation_name','designation','mobile_no','country', 'dls_officer_registration_no', 'bvc_officer_registration_no')
...
class OrganizationForm(forms.ModelForm):
    ...
    organization_type = forms.IntegerField(label='Organization Type')
    ...
    class Meta:
        model = Organizations
        fields = ('organization','parent_organization','organization_type')
