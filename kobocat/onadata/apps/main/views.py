...
def check_valid_user(request):
    ...
    validation_query = "select geoid from organization_catchment_area where organization_id = (select organisation_name_id from usermodule_usermoduleprofile where user_id = (select id from auth_user where username='"+str(username)+"'))"
    validation_data = __db_fetch_single_value(validation_query)
    return HttpResponse(validation_data)


@csrf_exempt
def check_valid_lab_user(request):
    reqbody = json.loads(request.body)
    username = reqbody['username']
    validation_query = "select organisation_name_id from usermodule_usermoduleprofile where user_id = (select id from auth_user where username='"+str(username)+"')"
    validation_data = __db_fetch_single_value(validation_query)
    return HttpResponse(validation_data)
...
def get_labs_list(request):
    lab_lists_query = "select id,organization from usermodule_organizations where organization_type = 3 and parent_organization_id = 69"
    lab_lists_data = __db_fetch_values_dict(lab_lists_query)
    return HttpResponse(json.dumps(lab_lists_data))


@csrf_exempt
def get_medicine_list(request):
    medicine_query = "with t as(select medicine_type || ' ' || trade_name|| ' ' || pac_size as mlist from medicine_list) select distinct t.mlist from t"
    medicine_data = __db_fetch_values(medicine_query)
    return HttpResponse(json.dumps(medicine_data))


@csrf_exempt
def get_symptoms_list(request):
    symptom_query = "select distinct symptoms from patients_symptoms"
    symptom_data = __db_fetch_values(symptom_query)
    return HttpResponse(json.dumps(symptom_data))


@csrf_exempt
def get_diseases_list(request):
    diseases_query = "select distinct disease_name from patients_disease"
    diseases_data = __db_fetch_values(diseases_query)
    return HttpResponse(json.dumps(diseases_data))
...
def get_user_id(request):
    reqbody = json.loads(request.body)
    username = reqbody['username']
    user_query = "select auth_user.id,usermodule_usermoduleprofile.designation_id from auth_user left join usermodule_usermoduleprofile on usermodule_usermoduleprofile.user_id = auth_user.id where auth_user.username = '"+str(username)+"' limit 1"
    user_id = __db_fetch_values(user_query)
    return HttpResponse(json.dumps(user_id))

@csrf_exempt
def get_user_access_data(request):
    access_query = "select designation_id,resource_id from designations_resource_map"
    access_data = __db_fetch_values_dict(access_query)
    return HttpResponse(json.dumps(access_data))


@csrf_exempt
def sync_ulo_forms_data(request):
    reqbody = json.loads(request.body)
    current_date = datetime.today().strftime('%Y-%m-%d')
    inser_query = "INSERT INTO public.forms_data (form_data_id, form_name, datajson, sync_status, sync_datetime, working_upazila_id) VALUES(" + str(
        reqbody['form_data_id']) + ", '" + str(reqbody['form_name']) + "', '" + reqbody['datajson'] + "', 0, '" + str(
        current_date) + "', " + str(reqbody['working_upazila_id']) + ") returning id"
    newid = __db_commit_query(inser_query)
    return HttpResponse(json.dumps({'sync_datetime': current_date, 'sync_status': 1, 'new_id':newid}))



@csrf_exempt
def get_designations(request):
    designation_query = "WITH RECURSIVE rec_d(id, designations_name,supervisor_id) AS ( SELECT fao_designations.id, fao_designations.designations_name,fao_designations.supervisor_id FROM fao_designations WHERE designations_name = 'ULO' UNION ALL SELECT fao_designations.id, fao_designations.designations_name,fao_designations.supervisor_id FROM rec_d, fao_designations where fao_designations.supervisor_id = rec_d.id) SELECT id, designations_name,supervisor_id FROM rec_d"
    designation_data = __db_fetch_values_dict(designation_query)
    return HttpResponse(json.dumps(designation_data))


@csrf_exempt
def get_sample_submissions(request):
    reqbody = json.loads(request.body)
    print reqbody
    lab_id = reqbody['lab_id']
    sample_data_query = "with t as(select id as data_id, datajson->>'date_sample_collected' as date_sample_collected, datajson->>'investigation_id' as investigation_id, datajson->>'division' as division, datajson->>'district' as district, datajson->>'upazila' as upazila, datajson->>'xunion' as xunion, datajson->>'mouza' as mouza, datajson->>'village' as village, datajson->>'contact_person_name' as contact_person_name, datajson->>'contact_person_mobile' as contact_person_mobile, datajson->>'species_name_id' as species_name_id, datajson->>'sonali_chicken_sample_type_description' as sonali_chicken_sample_type_description, datajson->>'deshi_chicken_sample_type_description' as deshi_chicken_sample_type_description, datajson->>'brown_comm_chicken_sample_type_description' as brown_comm_chicken_sample_type_description, datajson->>'brown_comm_chicken_sample_type_description' as white_comm_chicken_sample_type_description, datajson->>'duck_sample_type_description' as duck_sample_type_description, datajson->>'other_sample_type_description' as other_sample_type_description, datajson->>'sonali_chicken_sample_ID' as sonali_chicken_sample_ID, datajson->>'deshi_chicken_sample_ID' as deshi_chicken_sample_ID, datajson->>'brown_comm_sample_ID' as brown_comm_sample_ID, datajson->>'white_comm_sample_ID' as white_comm_sample_ID, datajson->>'duck_sample_ID' as duck_sample_ID, datajson->>'other_sample_ID' as other_sample_ID, datajson->>'lab_info' as lab_info, datajson->>'other' as other from forms_data where form_name = 'Avian Influenza Sample') select data_id, Coalesce( deshi_chicken_sample_ID, sonali_chicken_sample_ID,duck_sample_ID,white_comm_sample_ID,other_sample_ID, brown_comm_sample_ID) as sample_id, Coalesce( sonali_chicken_sample_type_description, deshi_chicken_sample_type_description,brown_comm_chicken_sample_type_description,white_comm_chicken_sample_type_description,duck_sample_type_description, other_sample_type_description) as sample_description, lab_info, investigation_id, date_sample_collected, division, district, upazila, xunion, mouza, village, contact_person_name, contact_person_mobile, case when species_name_id = 'Other' then other else species_name_id end as species_name_id from t where lab_info::int = "+str(lab_id)
    sample_data = __db_fetch_values_dict(sample_data_query)
    return HttpResponse(json.dumps(sample_data))

@csrf_exempt
def get_sink_submissions(request):
    reqbody = json.loads(request.body)
    lab_id = reqbody['lab_id']
    sample_data_query = "WITH q AS(SELECT id, form_data_id, datajson ->> 'live_bird_market_id' AS live_bird_market_id, datajson ->> 'sample_collected_by' AS sample_collected_by, datajson ->> 'market_name' AS market_name, datajson ->> 'city_north_or_south' AS city_north_or_south, datajson ->> 'A_lab_identification' AS sample_id, datajson ->> 'date_sample_collected' AS date_sample_collected FROM forms_data WHERE form_name = 'Sink Surveillance' and working_upazila_id = "+str(lab_id)+" UNION SELECT id, form_data_id, datajson ->> 'live_bird_market_id' AS live_bird_market_id, datajson ->> 'sample_collected_by' AS sample_collected_by, datajson ->> 'market_name' AS market_name, datajson ->> 'city_north_or_south' AS city_north_or_south, datajson ->> 'S_lab_identification' AS sample_id, datajson ->> 'date_sample_collected' AS date_sample_collected FROM forms_data WHERE form_name = 'Sink Surveillance' and working_upazila_id = "+str(lab_id)+" UNION SELECT id, form_data_id, datajson ->> 'live_bird_market_id' AS live_bird_market_id, datajson ->> 'sample_collected_by' AS sample_collected_by, datajson ->> 'market_name' AS market_name, datajson ->> 'city_north_or_south' AS city_north_or_south, datajson ->> 'E_lab_identification' AS sample_id, datajson ->> 'date_sample_collected' AS date_sample_collected FROM forms_data WHERE form_name = 'Sink Surveillance' and working_upazila_id = "+str(lab_id)+") SELECT id AS data_id, form_data_id, live_bird_market_id, sample_collected_by, market_name, CASE WHEN city_north_or_south :: INT = 3 THEN 'CCC' WHEN city_north_or_south :: INT = 2 THEN 'DSCC' WHEN city_north_or_south::INT = 1 THEN 'DNCC' END AS city_corp, sample_id, date_sample_collected FROM q WHERE sample_id IS NOT null"
    sample_data = __db_fetch_values_dict(sample_data_query)
    return HttpResponse(json.dumps(sample_data))
...
@csrf_exempt
def sync_result_inputs_data(request):
    reqbody = json.loads(request.body)
    print reqbody
    reference_desktop_id = reqbody['id']
    del reqbody['id']
    reqbody['reference_desktop_id'] = reference_desktop_id 
    keys = reqbody.keys()
    columns = ','.join(keys)
    values = ','.join(['%({})s'.format(k) for k in keys])
    insert = 'insert into result_inputs ({0}) values ({1})'.format(columns, values)
    cursor = connection.cursor()
    insert_qry = cursor.mogrify(insert, reqbody)
...
@csrf_exempt
def sync_lab_results_to_ulo(request):
    reqbody = json.loads(request.body)
    ulo_id = reqbody['ulo_id']
    ulo_results_query = "select id, reference_desktop_id, reference_sample_id, reference_investigation_id, result, test_setup_by, test_set_up_reference_no, name_result_input_by, rt_pcr_type_test, rt_pcr_threshold, result_date, test_set_up_number, protocol_details, date_test_set_up, sero_type_n, sero_type_h, test_name, remarks, date_out_put, rt_pcr_control_value_2, rt_pcr_control_value_1, other_test_detail, other_test_name, ct_value, result_details from result_inputs where reference_investigation_id in (select distinct on(investigation_id) investigation_id from vw_avian_influenza_investigation where working_upazila_id = "+str(ulo_id)+")"
    ulo_data = __db_fetch_values_dict(ulo_results_query)
    return HttpResponse(json.dumps(ulo_data,default=json_util.default))


@csrf_exempt
def sync_back_lab_results_to_lab(request):
    reqbody = json.loads(request.body)
    lab_id = reqbody['lab_id']
    lab_results_query = "SELECT id, reference_sample_id, reference_investigation_id, result, test_setup_by, test_set_up_reference_no, name_result_input_by, rt_pcr_type_test, rt_pcr_threshold, result_date, test_set_up_number, protocol_details, date_test_set_up, sero_type_n, sero_type_h, test_name, remarks, date_out_put, rt_pcr_control_value_2, rt_pcr_control_value_1, other_test_detail, other_test_name, ct_value, result_details FROM public.result_inputs where lab_id = "+str(lab_id)
    lab_results_data = __db_fetch_values_dict(lab_results_query)
    return HttpResponse(json.dumps(lab_results_data,default=json_util.default))    


@csrf_exempt
def get_ai_sample_submissions(request):
    reqbody = json.loads(request.body)
    lab_id = reqbody['lab_id']
    ai_sample_data_query = "select datajson from forms_data where form_name = 'Avian Influenza Sample' and datajson->>'lab_info' = '"+str(lab_id)+"'"
    ai_sample_data = __db_fetch_values_dict(ai_sample_data_query)
    return HttpResponse(json.dumps(ai_sample_data,default=json_util.default))


@csrf_exempt
def get_sync_back(request):
    reqbody = json.loads(request.body)
    ulo_id = reqbody['ulo_id']
    print ulo_id
    get_sync_back_query = "select * from forms_data where working_upazila_id = "+str(ulo_id)
    get_sync_back_data = __db_fetch_values_dict(get_sync_back_query)
    return HttpResponse(json.dumps(get_sync_back_data,default=json_util.default))


@csrf_exempt
def sync_device_id(request):
    reqbody = json.loads(request.body)
    node_id = reqbody['node_id']
    mac_address = reqbody['mac_address']
    check_device_exist = __db_fetch_values("SELECT ds from fao_device where mac_address = '"+str(mac_address)+"' and node_id = "+str(node_id))
    if not check_device_exist:
        d_count = __db_fetch_single_value("SELECT count(*) FROM public.fao_device where node_id = "+str(node_id))
        device_id = __db_commit_query("INSERT INTO public.fao_device (node_id, mac_address, created_at,ds) VALUES("+str(node_id)+", '"+str(mac_address)+"', now(),"+str(d_count+1)+") returning id")
        return HttpResponse(d_count+1)
    else:
        return HttpResponse(check_device_exist[0][0])



@csrf_exempt
def sync_lab_tests(request):
	reqbody = json.loads(request.body)
	test_setup_data = reqbody['test_setup_data']
	test_setup_data['lab_id'] = reqbody['lab_id']
	test_setup_data['sync_status'] = 0
	ts_insert_query = "INSERT INTO tests_setup (" + ", ".join(test_setup_data.keys()) + ") VALUES (" + ", ".join(["%("+k+")s" for k in test_setup_data]) + ") returning id;"
	cursor = connection.cursor()
	cursor.execute(ts_insert_query,test_setup_data)
	connection.commit()
	fetchVal = cursor.fetchone()
	cursor.close()
...
@csrf_exempt
def get_previous_tests_data(request):
	reqbody = json.loads(request.body)
	lab_id = reqbody['lab_id']
	previous_tests_query = "select id, date_output, sero_type_n, date_setup, rt_pcr_control_value_2, sero_type_h, test_setup_reference_number, protocol_details, rt_pcr_threshold, rt_pcr_control_value_1, other_test_detail, test_name, other_test_name, test_setup_by, rt_pcr_type_test_a from tests_setup where lab_id = "+str(lab_id)
	previous_tests_data = __db_fetch_values_dict(previous_tests_query)
	return HttpResponse(json.dumps(previous_tests_data,default=json_util.default))


@csrf_exempt
def get_previous_ai_sample_data(request):
	reqbody = json.loads(request.body)
	lab_id = reqbody['lab_id']
	previous_ai_sample_query = "select id, sample_id, received_sample_id, sample_inestigation_id, physical_sample_quality, sample_type, sample_status, sample_received_by, date_sample_collected from sample_finals where lab_id = "+str(lab_id)
	previous_ai_sample_data = __db_fetch_values_dict(previous_ai_sample_query)
	return HttpResponse(json.dumps(previous_ai_sample_data,default=json_util.default))


@csrf_exempt
def sync_ai_sample_from_lab(request):
	reqbody = json.loads(request.body)
	ai_sample_data = reqbody['ai_sample_data']
	ai_sample_data['lab_id'] = reqbody['lab_id']
	ai_sample_data['sync_status'] = 0
	ai_sample_insert_query = "INSERT INTO sample_finals (" + ", ".join(ai_sample_data.keys()) + ") VALUES (" + ", ".join(["%("+k+")s" for k in ai_sample_data]) + ") returning id;"
	cursor = connection.cursor()
	cursor.execute(ai_sample_insert_query,ai_sample_data)
	connection.commit()
	fetchVal = cursor.fetchone()
	cursor.close()
	return HttpResponse(fetchVal[0])
...
@csrf_exempt
def get_live_bird_markets(request):
	reqbody = json.loads(request.body)
	last_sync_date = reqbody['last_sync_date']
	
	#last_sync_date = datetime.strptime(last_sync_date, '%Y-%m-%d')
	get_markets_query = "select market_id,market_name,ward_number,city_corp,assigned_officer from livebird_markets"
	get_markets_data = __db_fetch_values_dict(get_markets_query)
	return HttpResponse(json.dumps(get_markets_data,default=json_util.default))
