...
def custom_data_view(request, username, id_string):
    ...
    if submission_start and submission_end is not '%':
        sub_query_date_range += " AND logger_instance.date_created BETWEEN '"+str(submission_start) + "' AND '" + str(submission_end) + "'"
    if status is not '%' and status!= 'custom':
        sub_query_status += " AND app_inst.status = '" + str(status) + "'"    


    if xform.id == 391:
        db_test_column_query = "logger_instance.id ID,json->>'WMG_ID' wmg_id, (select value_label from xform_extracted where xform_id=391 and field_name='district' and value_text=json->>'district') district,json->>'polder' polder, (select value_label from xform_extracted where xform_id=391 and field_name='name_wmg' and value_text=json->>'name_wmg' limit 1) name_wmg,to_char(date_created,'dd-Mon-yyyy') received "
        column_query = db_test_column_query
        ACTIVATE_CUSTOM_VIEW_QUERY = True   

    #if xform.id == 248:
    elif xform.id_string == 'np_VCPC_profile':
    ...
    elif xform.id_string == 'np_quarterly_case_study':
        db_test_column_query = "logger_instance.id ID, logger_instance.user_id ,  to_char(logger_instance.date_created,'DD Mon YYYY') Received, logger_instance.json->>'basic/pngo' PNGO, app_inst.status ApprovalStatus"
        column_query = db_test_column_query
        ACTIVATE_CUSTOM_VIEW_QUERY = True 
    else :
        db_test_column_query = "logger_instance.id ID, logger_instance.user_id ,  to_char(logger_instance.date_created,'DD Mon YYYY') Received "
        column_query = db_test_column_query
        ACTIVATE_CUSTOM_VIEW_QUERY = True

    #submission_instance_query = "SELECT DISTINCT " + column_query + " FROM logger_instance LEFT JOIN approval_instanceapproval app_inst ON app_inst.instance_id = logger_instance.id WHERE xform_id = " + str(xform.id) + str(sub_query_user) + str(sub_query_date_range) + str(sub_query_status)

    submission_instance_query = "SELECT DISTINCT " + column_query + " FROM logger_instance WHERE xform_id = " + str(xform.id) + str(sub_query_user) + str(sub_query_date_range) + str(sub_query_status)
