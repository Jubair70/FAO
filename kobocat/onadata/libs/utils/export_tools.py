...
from onadata.libs.utils.db_utils import DbInstance
...
SINGLE_SELECT_BIND_TYPE = u"select1"
...
class ExportBuilder(object):
    ...
    if settings.SHOW_EXPORT_METADATA is True:
    ...
    def set_survey(self, survey):
            ...
            for child in survey_element.children:
                ...
                if isinstance(child, Section):
                    ...
                    if isinstance(child, Question):
                        if child.bind.get(u"type") == SINGLE_SELECT_BIND_TYPE and self.SHOW_LABEL:
        ...
        self.childSheet = False
        self.survey = survey
        self.select_multiples = {}
        self.gps_fields = {}
        self.encoded_fields = {}
        self.select_one = {}
        self.select_label_map = {}
        main_section = {'name': survey.name, 'elements': []}
        self.sections = [main_section]
        build_sections(
            main_section, self.survey, self.sections,
            self.select_multiples, self.gps_fields, self.encoded_fields, self.select_one, self.select_label_map,
    ...
    @classmethod
    def show_select_label(cls,row,select_one,label_map,label_lang=None):
                ...
                for select in selections:
                    try:
                        if label_lang is not None:
                            choice_label = label_map[select][label_lang]
                        else:
    ...
    @classmethod
    def split_select_multiples(cls, row, select_multiples, label_map, show_label,label_lang=None):
    ...
    def pre_process_row(self, row, section):
        ...
        if section_name in self.select_one and self.SHOW_LABEL:
            if self.LABEL_LANG is not None:
                row = ExportBuilder.show_select_label(row, self.select_one[section_name], self.select_label_map, self.LABEL_LANG)
            else:
        ...
        if self.SPLIT_SELECT_MULTIPLES and\
            ...
            if self.LABEL_LANG is not None:
                row = ExportBuilder.split_select_multiples(row, self.select_multiples[section_name], self.select_label_map, self.SHOW_LABEL,self.LABEL_LANG)
            else:
                row = ExportBuilder.split_select_multiples(row, self.select_multiples[section_name], self.select_label_map, self.SHOW_LABEL)
    ...
    def to_postgres_sav(self, path, data, *args):
        ...
        for section in self.sections:
            ...
            if section_count > 1:
                db_table_name = self.survey.name + '_' + work_sheet_title
            else:
                db_table_name = self.FORM_OWNER + '_' + work_sheet_title
            work_sheet_titles[section_name] = db_table_name
            work_table_name[section_name] = db_table_name
            instance_db.create_form_db_table(db_table_name)
            section_count += 1
    ...
    def to_xls_export(self, path, data, *args):
        ...
        print type(self.DEFAULT_XLS_FIELDS)
        print self.DEFAULT_XLS_FIELDS
        if len(eval(self.DEFAULT_XLS_FIELDS)) > 0:
            constant_fields = [unicode(i.replace("'","")) for i in self.DEFAULT_XLS_FIELDS.strip('[]').split(', ')]
        ...
        def write_row(data, work_sheet, fields, work_sheet_titles):
            ...
            if constant_fields:
                if all(x in fields for x in constant_fields):
                    for cf in constant_fields:
                        constant_dicts[cf] = data.get(cf)
                else:
                    if constant_dicts:
                        for cfx in constant_fields:
                            constant_values.append(constant_dicts[cfx])
            # update parent_table with the generated sheet's title
            data[PARENT_TABLE_NAME] = work_sheet_titles.get(
                data.get(PARENT_TABLE_NAME))


                    #work_sheet.append(constant_dicts[cfx])
            work_sheet.append(constant_values + [data.get(f) for f in fields])
        ...
        for section in self.sections:
            ...
            if self.SHOW_LABEL:
                ...
                for element in section['elements']:
                    if element['label'] == '':
                        headers.append(element['xpath'])
                        if constant_fields is not None and element['xpath'] in constant_fields:
                            header_dicts[element['xpath']] = element['xpath']
                    else:
                        if isinstance(element['label'],dict):
                            headers.append(element['label'][self.LABEL_LANG])
                            if constant_fields is not None and element['xpath'] in constant_fields:
                                header_dicts[element['xpath']] = element['label'][self.LABEL_LANG]
                        else:
                            headers.append(element['label'])
                            if constant_fields is not None and element['xpath'] in constant_fields:
                                header_dicts[element['xpath']] = element['label']

                if constant_fields is not None and all(x in [element['xpath'] for element in section['elements']] for x in constant_fields):
                    self.childSheet = False
                else:
                    self.childSheet = True

                if self.childSheet is True:
                    const_headers = []
                    if constant_fields:
                        const_headers = [header_dicts[str(p)] for p in constant_fields]
                    headers = const_headers + headers + self.EXTRA_FIELDS
                else:
                    headers = headers + self.EXTRA_FIELDS
            else:
                headers = []
                for element in section['elements']:
                    headers.append(element['title'])
                    if constant_fields is not None and element['xpath'] in constant_fields:
                        header_dicts[element['xpath']] = element['title']
                ...
                if constant_fields is not None and self.childSheet is True:
                    headers = constant_fields + headers + self.EXTRA_FIELDS
                else:
                    headers = headers + self.EXTRA_FIELDS
            # get the worksheet
...
def generate_export(export_type, extension, username, id_string,
    ...
    export_builder.LABEL_LANG = label_lang
    export_builder.DEFAULT_XLS_FIELDS = default_xls_fields
