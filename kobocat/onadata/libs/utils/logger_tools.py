...
def check_custom_form_validation(xml,request,username):
    ...
    if form_id != FORM_ID_TO_CHECK:
        return True
    all_Tag_to_parse = collection.getElementsByTagName("group_arsenic_level_group")
...
def remove_xform(xform):
    ...
    xform_instances.remove(query, j=True)
     ...
     # delete all approval information
    remove_approval(xform)
...
def remove_approval(xform):
    # delete all approvalDef data related to current xform
    approval_def = ApprovalDef.objects.filter(formid=xform.id_string)
    approval_def.delete()

    # delete data from approval table
