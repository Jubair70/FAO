...
function createTable(canEdit,datavals) {
    ...
    if (canEdit === true) {
        var editDelete = '<div class="row"><div class="col-md-6"><a target="_blank" id="title_edit" href="#kate" class="btn red bind-edit disabled">' + gettext("edit") + '</a>&nbsp;<a href="#"class="btn red btn-danger">' + gettext("Delete") + '</a></div></div><div class="top-buffer-min">';
        dataContainer.append(editDelete);
    }
    var notesSection = '<div id="notes" style="display: none;> \
    <form action="" onsubmit="return addOrEditNote()" method="post" name="notesform" id="notesform"> \
        ...
        placeholder="' + gettext("Add note to instance") + '" autocomplete="off" style="width: 100%"></textarea> \
        </div> \
        <div class="controls controls-row"> \
        <div class="top-buffer-min"> \
        <button type="submit" id="note_save" class="btn green" >' + gettext("Save note") + '</button> \
        </div>\
    </form> \
    <div id="notes-section"></div></div><div class="top-buffer">';
...
function deleteData(context, data_id, redirect_route) {
	...
	$.ajax({
	   ...
	   window.location.href = '/usermodule/cupadmin/projects-views/Urban_Programme_2016_Baseline_Survey_2/';	
	}
	});
}
